function [f] = X_Plane_PLOS(k_pp_psi, k_pp_th, k_plo_psi, k_plo_th, k_t)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% X-Plane simulation using the PLOS algorithm
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% MAIN LOOP %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    while counter <= iterations && error_flag == 1
        ...
        % PLOS Algorithm
        [roll_d, pitch_d, error] = PLOS(UAV_position, loiter_position, radius, yaw(counter), pitch(counter), k_pp_psi, k_pp_th, k_plo_psi, k_plo_th, k_t, UAV_speed(counter), dt, PLOS_counter);
        ...
        % Compute benchmarks
        Error(counter) = sqrt((error(1) - trajectory(4))^2 + error(3)^2);
        partial_error = partial_error + Error(counter)^2; 
        error_x(counter) = error(1) - trajectory(4);
        ...
        % Update PLOS counter for Gain Scheduling strategy
        if counter >= 200
        ...
        dt = mean(intervalo_real_iter(1:counter));
